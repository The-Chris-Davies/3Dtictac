...
void move(){
	...
	while(true){
		...
		for(byte i = 0; i < 3; i++){
			pos[i] +=  moveDir[i];
			pos[i] = ((pos[i] == 255)?(pos[i]-1):(pos[i])) % 4;
			moveDir[i] = 0;
		}
	}
}
...
void redraw(){
	...
	//set cursor pixels
	Rb.setPixelZXY(pos[2],pos[0],pos[1],0xFF7000);
}
...
void winner(char player){
	...
	delay(2000);
	for(byte x=0;x<50;x++){
		Rb.setPixelZXY(random(4),random(4),random(4),0x00FF00<<(256*player)); //uses 24-bit RGB color
		delay(100);
	}
}
